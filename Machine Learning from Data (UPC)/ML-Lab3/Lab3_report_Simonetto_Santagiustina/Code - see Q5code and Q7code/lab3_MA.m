...
clear ref
% options
V_clases=[1 2 3 4 6 7 9 12];            % vector with selected classesVector de CLASES SELECCIONADAS
                                        % that is, classes that contain more tan two vectors
i_dib=1;                                %0 NO /1 YES: plot vectors
i_dist=1;                               % 1 compute distances between elements and classes
P_train=0.5;                            % percentage of training elements                  
K_neig=input('valor de K_neig = ');     % parameter K in knnc
N_feat=6830;
%% Database partition
Index_train=[];
Index_test=[];
...
if i_dib==1
    figure('name','Microarray per class')
    for i_class=1:length(V_clases);
        index=find(Labels==V_clases(i_class));
        subplot(4,2,i_class)
        hold on
