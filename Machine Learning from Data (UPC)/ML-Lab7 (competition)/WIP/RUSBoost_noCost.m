...
for i = 1:length(l_rate)
    ...
    for j = 1:num_folds
        t = templateTree();
        cla = fitcensemble(xtrain(cv.training(j),:),lab_train(cv.training(j)),'Method','RUSBoost','Learners',t,'LearnRate',l_rate(i));
...
%%
cl = fitcensemble(xtrain,lab_train,'Method','RUSBoost','Learners',t,'LearnRate',best_learn);
