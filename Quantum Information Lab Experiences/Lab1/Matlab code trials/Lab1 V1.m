...
count = 0;
 ...
 title 'Histogram of time differences between arrival at ch1 and 2'
 xlabel 'seconds'
...
coincidence_time = mean(diff_filt)+1*sqrt(var(diff_filt));

%% -------------Counting coincidences
coincidences=0;
if coincidence_time < 2*10.^(-9)
    for i=1:count
        if diff_filt(i)<coincidence_time
            coincidences = coincidences+1;
        end
    end
else
    for i=1:n_pairs
        if diff(i)<coincidence_time
            coincidences = coincidences+1;
        end
    end
end
...
corrD=probC(1,1)+probC(2,2)-probC(1,2)-probC(2,1);
...
for i=1:2
    for j=1:2
err_blockA(i,j)=blockA(i,j).*((sum(sum(blockA))-blockA(i,j))/((sum(sum(blockA))).^2)).^2;
    end
end
...
for i=1:2
    for j=1:2
err_blockB(i,j)=blockB(i,j).*((sum(sum(blockB))-blockB(i,j))/((sum(sum(blockB))).^2)).^2;
    end
end
...
for i=1:2
    for j=1:2
err_blockC(i,j)=blockC(i,j).*((sum(sum(blockC))-blockC(i,j))/((sum(sum(blockC))).^2)).^2;
    end
end
...
for i=1:2
    for j=1:2
err_blockD(i,j)=blockD(i,j).*((sum(sum(blockD))-blockD(i,j))/((sum(sum(blockD))).^2)).^2;
    end
end
...
if S-sigma_S>2
    ['S = ', num2str(S),'  > 2 and sigma_S = ', num2str(sigma_S), ' so the Bell inequalities have been broken with high confidence.']
